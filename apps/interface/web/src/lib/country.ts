...
/**
 * Normalize an alpha-2 or alpha-3 country code to alpha-2.
 */
function toAlpha2(code: string): string | null {
  const upper = code.toUpperCase();
  const a2 = upper.length === 3 ? alpha3to2[upper] : upper;
  if (!a2 || a2.length !== 2 || !/^[A-Z]{2}$/.test(a2)) return null;
  return a2;
}
 ...
 */
export function countryToFlag(code: string): string | null {
  const a2 = toAlpha2(code);
  if (!a2) return null;
  return String.fromCodePoint(
    ...[...a2].map((ch) => 0x1f1e6 + ch.charCodeAt(0) - 65),
  );
}
...
/**
 * Get the full country name from an alpha-2 or alpha-3 code.
 */
export function countryName(code: string): string | null {
  const a2 = toAlpha2(code);
  if (!a2) return null;
  try {
    return displayNames.of(a2) ?? null;
  } catch {
    return null;
  }
}
