-- Migration: Add materialized views for users, sessions, metrics_daily, property_metadata
-- The old tables used incompatible engines/column types for aggregation.
-- This migration drops and recreates them with AggregatingMergeTree + -State columns,
-- then creates materialized views to auto-populate from events inserts.
--
-- WARNING: This drops all data in users, sessions, metrics_daily, and property_metadata.
-- Historical data can be backfilled by re-inserting events or using INSERT ... SELECT.

-- ════════════════════════════════════════════════
-- Step 1: Drop existing materialized views (idempotent)
-- ════════════════════════════════════════════════
DROP VIEW IF EXISTS analytics.mv_users;
DROP VIEW IF EXISTS analytics.mv_sessions;
...
-- ════════════════════════════════════════════════
-- Step 3: Recreate tables with AggregatingMergeTree
-- ════════════════════════════════════════════════
...
CREATE TABLE analytics.property_metadata (
    project_id String,
...
CREATE MATERIALIZED VIEW analytics.mv_users
TO analytics.users
AS
SELECT
    project_id,
...
-- MVs: Daily metrics — one per dimension

CREATE MATERIALIZED VIEW analytics.mv_metrics_overall
...
CREATE MATERIALIZED VIEW analytics.mv_metrics_event_name
TO analytics.metrics_daily
AS
SELECT
    project_id,
...
CREATE MATERIALIZED VIEW analytics.mv_metrics_browser
TO analytics.metrics_daily
AS
SELECT
    project_id,
...
CREATE MATERIALIZED VIEW analytics.mv_metrics_os
TO analytics.metrics_daily
AS
SELECT
    project_id,
...
CREATE MATERIALIZED VIEW analytics.mv_metrics_device_type
TO analytics.metrics_daily
AS
SELECT
    project_id,
...
CREATE MATERIALIZED VIEW analytics.mv_metrics_platform
TO analytics.metrics_daily
AS
SELECT
    project_id,
...
CREATE MATERIALIZED VIEW analytics.mv_metrics_country
TO analytics.metrics_daily
AS
SELECT
    project_id,
...
CREATE MATERIALIZED VIEW analytics.mv_metrics_path
TO analytics.metrics_daily
AS
SELECT
    project_id,
...
-- MVs: Property metadata — one per property type

CREATE MATERIALIZED VIEW analytics.mv_property_metadata_str
...
CREATE MATERIALIZED VIEW analytics.mv_property_metadata_num
TO analytics.property_metadata
AS
SELECT
    project_id,
...
CREATE MATERIALIZED VIEW analytics.mv_property_metadata_bool
TO analytics.property_metadata
AS
SELECT
    project_id,
